        ...
        while current_time_cursor <= end_journey_dt:
            ...
            if is_position_active:
                ...
                if is_sl_breached or is_expiry_end:
                    ...
                    result_row = {
                        "trade_id":            trade_id,
                        "chain":               chain_counter,       # reentry number
                    ...
                    print(
                        f"[{trade_id}] Chain#{chain_counter} {entry_date_val} | "
                    ...
                    # ----------------------------------------------------------
                    if is_expiry_end:
                        # Expiry ho gayi — poori journey khatam
                        is_position_active = False
                        active_strikes     = None
                        break

                    # SL hit — same candle pe reentry
                    # Step 1: Exit spot price nikalo (jis minute SL hua)
                    reentry_spot = fetching_spot_price(current_time_cursor)

                    if not reentry_spot:
                        # Spot nahi mila — skip reentry, next candle try karo
                        is_position_active = False
                        active_strikes     = None
                        current_time_cursor += timedelta(minutes=1)
                        continue

                    # Step 2: Exit spot se nayi strikes calculate karo
                    active_strikes   = get_strikes(reentry_spot)
                    entry_spot_price = reentry_spot

                    # Step 3: Same minute ka nayi strikes ka premium fetch karo
                    p_reentry = fetch_single_minute_premiums(
                        active_strikes, current_time_cursor, expiry_datetime)

                    if not all(k in p_reentry for k in ["CE_Main", "PE_Main", "CE_Hedge", "PE_Hedge"]):
                        # Premium nahi mila — skip reentry, next candle try karo
                        is_position_active = False
                        active_strikes     = None
                        current_time_cursor += timedelta(minutes=1)
                        continue

                    # Step 4: Reentry — slippage lagao nayi entry pe
                    sh_ce_entry = apply_slippage(p_reentry["CE_Main"],  is_entry=True, is_short=True)
                    sh_pe_entry = apply_slippage(p_reentry["PE_Main"],  is_entry=True, is_short=True)
                    by_ce_entry = apply_slippage(p_reentry["CE_Hedge"], is_entry=True, is_short=False)
                    by_pe_entry = apply_slippage(p_reentry["PE_Hedge"], is_entry=True, is_short=False)
                    ...
                    print(
                        f"         ↳ Reentry Chain#{chain_counter} @ {current_time_cursor.strftime('%H:%M')} | "
                        f"Spot: {round(reentry_spot, 2)} | "
                        f"Strikes CE:{active_strikes['CE_Main']} PE:{active_strikes['PE_Main']}"
                    )

                    # Same minute me hi reentry ho gayi — cursor advance mat karo
                    # Next iteration mein yahi minute monitor hoga (SUB-FLOW B)
                    continue
